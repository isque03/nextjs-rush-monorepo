...
- a **legacy ExtJS application** (separate package with minified build)
...
│   │   └── package.json
│   └── legacy-extjs-app/    # Legacy ExtJS application
│       ├── src/             # ExtJS source code
...
2. **Linking local projects** together (e.g., `web-app` depends on `shared-lib` and `legacy-extjs-app`)
...
If you only want to build a specific project:

```bash
rush build --to @myorg/shared-lib
rush build --to @myorg/legacy-extjs-app
```

Each project's build command is defined in its `package.json` under the `scripts.build` field:
- **shared-lib**: Runs `tsc --build`, outputs compiled JavaScript and type definitions into `dist/`
- **legacy-extjs-app**: Runs `node build.js`, uses esbuild to minify ExtJS code into `dist/legacy-app.min.js`
...
- `/` - Home page
- `/legacy` - Legacy ExtJS application embedded in React
- `/react-demo` - React demo page (example of migrated screen)
...
## Working with the Legacy ExtJS Application

The legacy ExtJS application (`libraries/legacy-extjs-app`) is a separate Rush package that demonstrates embedding a legacy Sencha/ExtJS application within a Next.js React shell.

### Architecture

The legacy app follows a clean separation pattern:
- **Source code** lives in `libraries/legacy-extjs-app/src/legacy-app.js`
- **Build process** minifies the code using esbuild to `dist/legacy-app.min.js`
- **Integration** copies the minified file to Next.js `public/legacy/` during build/dev
- **Runtime** Next.js serves the file as a static asset, loaded via script tag

### Building the Legacy App

The legacy app uses esbuild for minification:
...
cd libraries/legacy-extjs-app
rushx build
```

This creates `dist/legacy-app.min.js` (minified from the source).

### Automatic Build and Copy

When you run `rushx dev` or `rushx build` in the web-app, the `predev`/`prebuild` hooks automatically:
1. Check if the legacy app is built
2. Build it automatically if missing (via `rush build --to @myorg/legacy-extjs-app`)
3. Copy the minified file from `node_modules/@myorg/legacy-extjs-app/dist/` to `public/legacy/`

This ensures the legacy app is always available when starting the Next.js server.

### How It Works

1. **Legacy App Package** (`libraries/legacy-extjs-app/`):
   - Contains ExtJS application code in `src/legacy-app.js`
   - Build script (`build.js`) uses esbuild to minify to `dist/legacy-app.min.js`
   - Exposes `window.Legacy` API with `mount(domId)` and `unmount()` methods

2. **Next.js Integration** (`apps/web-app/src/app/legacy/page.tsx`):
   - Client component that dynamically loads ExtJS from CDN
   - Loads the minified legacy app from `/legacy/legacy-app.min.js`
   - Mounts the ExtJS app into a React-controlled container
   - Handles cleanup on unmount

3. **Build Pipeline**:
   - Rush builds the legacy package → `dist/legacy-app.min.js`
   - Copy script runs before Next.js build/dev → copies to `public/legacy/`
   - Next.js serves the file as a static asset

### Development Workflow

1. Make changes to `libraries/legacy-extjs-app/src/legacy-app.js`
2. Rebuild: `rush build --to @myorg/legacy-extjs-app`
3. The copy script will automatically copy the new build on next `rushx dev`
4. Or manually copy: `node apps/web-app/scripts/copy-legacy-app.js`

### Example: Adding a Real ExtJS App

To swap in a real ExtJS application bundle:

1. Replace `libraries/legacy-extjs-app/src/legacy-app.js` with your ExtJS app code
2. Ensure it exposes the same API: `window.Legacy = { mount, unmount }`
3. Rebuild: `rush build --to @myorg/legacy-extjs-app`
4. The existing integration will work without changes

This pattern proves that React can host any ExtJS/Sencha application, with React controlling the lifecycle and routing.
...
- **Legacy-extjs-app** is a separate package demonstrating how to embed legacy ExtJS/Sencha applications within React
...
- **Automatic build/copy** ensures legacy app is always available when starting Next.js
- Production builds use optimized compiled output from `dist/`

This starter serves as a clear example of how to organize a scalable monorepo where multiple applications, shared libraries, and legacy applications can coexist under a single consistent build system with fast development iteration. It demonstrates an incremental migration strategy where React hosts legacy ExtJS applications, proving the architecture for gradually migrating legacy codebases.
